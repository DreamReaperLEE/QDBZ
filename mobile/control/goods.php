...
class goodsControl extends mobileHomeControl{
    ...
    public function goods_listOp() {
        ...
        //处理商品列表(抢购、限时折扣、商品图片)
        $model_goods1 = Model('goods');
        //$goods_list =$model_goods1->getGoodsInfo($condition1);
        foreach ($goods_list as $key => $value){
            $goods_list[$key]['goods_price'] = $model_goods1->getGoodsNewPrice( $goods_list[$key]);;
            $goods_list[$key]['goods_promotion_price'] = $model_goods1->getGoodsNewPrice( $goods_list[$key]);;
            //2018.4*/
        }
