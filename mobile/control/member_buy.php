...
class member_buyControl extends mobileMemberControl {
    ...
    public function buy_step1Op() {
        ...
        $model_mobilemember = Model('member');
//        $update_mobiel['weixin_info'] = $_POST['spec_data'];

        $model_member1 = Model('member');
        //        $logic_member1 = logic('member');
//        $logic_member1->setMembercookie($this->member_info['member_id'],$_POST['spec_data']);//2018.4 从app前台传来的spec_data 规格信息,,含日期等
        $logic_buy = logic('buy');

        //得到会员等级
        $model_member = Model('member');
        $model_member->setMembercookie($this->member_info['member_id'],$_POST['spec_data']);

        $member_info = $model_member->getMemberInfoByID($this->member_info['member_id']);


//        $update_mobiel['weixin_info'] = $_POST['spec_data'];
//        $model_mobilemember->editMember($member_info,$update_mobiel);
//
    ...
    public function buy_datepriceOp(){
        $begindate = $_POST['startDate'];
        $enddate = $_POST['endDate'];
        $conditon['goods_id'] =$_POST['cart_id'] ;
...
//没有到储值卡！
        ...
        output_data(array('pay_sn' => $result['data']['pay_sn'],'payment_code'=>$order_info['payment_code'],'pay' =>$pay['member_pd'] ));
    }
