...
//        } else {
           ...
           // exit('支付方式提交错误');
            ...
            $post['pd_pay'] = 1;
            $pay_info = $this->_get_real_order_info($pay_sn,$post);
 ...
 * */
    ...
    public function check_passOp(){
        @header("Content-type: text/html; charset=".CHARSET);
        $password = $_POST['password'];
        $model_member = Model('member');
        $buyer_info = $model_member->getMemberInfoByID($this->member_info['member_id']);
        $check =  array();
        if ($buyer_info['member_paypwd'] == '' || $buyer_info['member_paypwd'] != md5($password)) {
            output_data(array('check'=>0));
        }
        else{
            output_data(array('check'=>1));
        }
    ...
    public function check_pdOp(){
        ...
        output_data(array('pay' =>$pay['member_pd'] ));
