...
class memberModel extends Model {
	...
	*/
    ...
    public function createSession($member_info = array(),$reg = false) {
        ...
        if($seller_info == null){
            $_SESSION['is_commonmember'] = 1;
        }
        else $_SESSION['is_commonmember'] = 0;
        $_SESSION['store_id'] = $seller_info['store_id'];
        $_SESSION['is_company'] = $seller_info['is_company'];
     ...
     * 写App会员的cookie
     * @param int $member_id
     * @param string $field 需要写的缓存键值, 例如：'*','member_name,member_sex'
     */
    public function setMembercookie($member_id,$cookie_info) {
       //wcache('cookie'+$member_id, $cookie_info, 'member_cookie');

        $this->editMember(array('member_id'=>$member_id),array('weixin_info'=>$cookie_info));
    }
    /**
     * d读App会员的cookie
     * @param int $member_id
     * @return array
     */
    public function getMembercookie($member_id) {
        //$cookie_info = rcache('cookie'+$member_id, 'member_cookie');
        $result =$this->getMemberInfoByID($member_id);
        $cookie_info =$result['weixin_info'];
        return $cookie_info;
    }
}
