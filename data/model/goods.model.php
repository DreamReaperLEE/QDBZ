...
class goodsModel extends Model{
    ...
    public function getGoodsList($condition, $field = '*', $group = '',$order = '', $limit = 0, $page = 0, $count = 0) {
        ...
        $result =  $this->table('goods')->field($field)->where($condition)->group($group)->order($order)->limit($limit)->page($page, $count)->select();
       foreach ($result as $v){
           $v['goods_price'] = $this->getGoodsNewPrice($v); //2018.4
        }


        return $result;
    }
    public function getGoodsgcid1List($condition, $field = '*') {
        $result = $this->table('goods')->field($field)->where($condition)->order('')->limit(false)->select();
        return $result;
    }


    public function getGoodsNewPrice($result1,$Stype =0){
        ...
        $total = 0;
            if($result['gc_id_1'] == '1'){
                if (APP_ID == 'mobile') {
                    ...
                    $tem =  $model_member1->getMembercookie($_SESSION['member_id']);


                    $data = json_decode(stripslashes(html_entity_decode($tem)) , true);

                    $begindate = $data['startDate'];
                    $enddate = $data['endDate'];
                    $Stype = $data['stype'];//=1

                    //$spec_data =$logic_member1->getMembercookie($this->member_info['member_id']);//2018.4 获取从app前台传来的spec_data 规格信息,含日期等
                    //$begindate = json_decode($spec_data)->start;
                    //$enddate =json_decode($spec_data)->end;


//                    $model_mobiel = Model('member');
//                    $mobile['member_id'] = $_SESSION['member_id'];
//                    $spec_data = $model_mobiel->getMemberCommonInfo($mobile['member_id']);
...
//                    return $total;
                    ...
                    $begindate = $_COOKIE['startDate'];
                    $enddate = $_COOKIE['endDate'];

                }
                else
                {
                    $begindate = json_decode($_COOKIE['getHotelPrice'])->start;
                    $enddate =json_decode($_COOKIE['getHotelPrice'])->end;
               ...
               $condition2['goods_id']= $result['goods_commonid'];
...
//                }
    ...
    public function getGoodsInfo($condition, $field = '*') {
        $result = $this->table('goods')->field($field)->where($condition)->find();
        $result['goods_price'] = $this->getGoodsNewPrice($result); //2018.4
        return $result;
    }
    public function getGoodinfo_new($condition, $field = '*'){
        $result = $this->table('goods')->field($field)->where($condition)->find();
        return $result;
    }//2018.4.6
    ...
    public function getGoodsCommonInfoByID($goods_commonid, $fields = '*') {
        /*$common_info = $this->_rGoodsCommonCache($goods_commonid, $fields); //2018.4 暂时不使用缓存*/
    ...
    public function getGoodsInfoByID($goods_id, $fields = '*') {
        //$goods_info = $this->_rGoodsCache($goods_id, $fields); //2018.4暂时不使用缓存

        $goods_info = $this->getGoodsInfo(array('goods_id'=>$goods_id));
        $this->_wGoodsCache($goods_id, $goods_info); //2018.4
        if (empty($goods_info)) {
            $goods_info = $this->getGoodsInfo(array('goods_id'=>$goods_id));
	...
	//好商城V5 专用放大镜
        ...
        //$goods_info["goods_price"] =$goods_info["goods_price"] ;//2018.4.5

        /*           $model_hotel = Model('goods');
            $condition_r['goods_id'] = 'goods_id';
                $v = $model_hotel->getGoodinfo_new($condition_r);
                $goods_info['goods_price'] = $model_hotel->getGoodsNewPrice($v); //2018.4*/
