...
class statModel extends Model{
    ...
    public function statByGoods($where, $field = '*', $page = 0, $limit = 0,$order = '', $group = '') {
        ...
        if (is_array($page)){
            if ($page[1] > 0){
                $result =  $this->table('goods')->field($field)->where($where)->group($group)->page($page[0],$page[1])->limit($limit)->order($order)->select();
            } else {
                $result = $this->table('goods')->field($field)->where($where)->group($group)->page($page[0])->limit($limit)->order($order)->select();
            }
        } else {
            $result =$this->table('goods')->field($field)->where($where)->group($group)->page($page)->limit($limit)->order($order)->select();



        }
        foreach ($result as $v){
            $v['goods_price'] = $model_hotel->getGoodsNewPrice($v);
        }
         //2018.4
        return $result;
    }
