...
class buyLogic {
    ...
    public function getBuyStep1Data($member_id, $data, $orderdiscount = 0) {
       /* if (APP_ID == 'mobile') {
        ...
        //输出用户默认收货地址
        $default_address_id = Model('address_member')->getDefaultAddressInfo(array('member_id'=>$member_id));
        $result['address_info'] = Model('address')->getAddressInfo(array('address_id'=>$default_address_id['address_id']));
...
//        }
    ...
    private function _createOrderStep3() {
        ...
        if ($this->_logic_buy_1->limitSpend($store_cart_list)) {
        ...
        $this->_order_data['store_rpt_total'] = $store_rpt_total;}
            ...
            throw new Exception('抱歉，您购买的部分商品超出您的限额！');
        }
    ...
    private function _createOrderStep4() {
        ...
        if (APP_ID == 'mobile'){
            ...
            if($seller_info == null){
                ...
                $buyer = $model_seller->getSellerInfo($condition);
                $condition = array(
                    'store_id' => $buyer['store_id'],
                );
                $store=$model_seller->getSellerList($condition);
                foreach ($store as $every){
                    if($every['is_admin'])
                        $member_id=$every['member_id'];
                }}
...
//             }
            ...
            foreach ($goods_list as $goods_info) {
                ...
                if (!intval($goods_info['bl_id'])) {
                    ...
                    $order_goods[$i]['goods_spec'] = $this->_post_data['spec_r'];
 //不同产品规格存放不一样，普通商品加判定；
