...
<ul class="cp-toast-list">
  ...
  <?php if ($output['statistics']['store_joinin'] > 0) {?>
        <li>
            <span>[保障中心-供应商]</span>
            <a target="workspace" href="<?php echo urlAdminshop('store', 'store_joinin');?>" onclick="openItem('shop|store')"><i class="fa fa-bell-o"></i>有<strong><?php echo $output['statistics']['store_joinin'];?></strong>条供应商开店申请需要处理。</a>
        </li>
    <?php }?>
    <?php if ($output['statistics']['store_joinin2'] > 0) {?>
        <li>
            <span>[保障中心-保障单位]</span>
            <a target="workspace" href="<?php echo urlAdminshop('company', 'store_joinin');?>" onclick="openItem('shop|store')"><i class="fa fa-bell-o"></i>有<strong><?php echo $output['statistics']['store_joinin2'];?></strong>条保障单位开店申请需要处理。</a>
        </li>
    <?php }?>
