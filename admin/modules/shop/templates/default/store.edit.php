...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?act=store&op=store" title="返回<?php echo $lang['manage'];?>列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
<h3>  <?php
          if ( $output['store_array']['is_company']== '1')
          {
              echo "保障单位管理 - 编辑会员";
          ...
          if ( $output['store_array']['is_company']== '1')
          {
              echo $lang['nc_company_manage_subhead'];
          }
          else
          {
              echo $lang['nc_store_manage_subhead'];
          }
          ?></h5>
          <!--
          <?php echo $lang['nc_store_manage'];?> - 编辑会员“<?php echo $output['joinin_detail']['member_name'];?>”的供应商信息
 ...
 <h3> <?php
    ...
    <ul class="tab-base nc-row">
      <li><a class="current" href="javascript:void(0);">相关信息</a></li>
    ...
    <form id="joinin_form" enctype="multipart/form-data" method="post" action="index.php?act=store&op=edit_save_joinin" style="display:none;">
      ...
      <table border="0" cellpadding="0" cellspacing="0" class="store-joinin">
        ...
        <tbody>
          <tr>
            <th>附件：</th>
            <td colspan="20"><input type="text" class="txt w300" name="enclosure_name" value="<?php echo $output['joinin_detail']['enclosure_name'];?>"></td>
          </tr>
          <tr>
            <th>附件电子版：</th>
            <td colspan="20"><a nctype="nyroModal"  href="<?php echo getStoreJoininImageUrl($output['joinin_detail']['enclosure_electronic']);?>"> <img src="<?php echo getStoreJoininImageUrl($output['joinin_detail']['enclosure_electronic']);?>" alt="" /> </a>
              <input type="file" name="enclosure_electronic"></td>
          </tr>
        </tbody>
      </table>
      <table border="0" cellpadding="0" cellspacing="0" class="store-joinin">
      ...
      <table border="0" cellpadding="0" cellspacing="0" class="store-joinin">
        ...
        <?php
        if($output['joinin_detail']['is_company']=='1')
        {
            echo '
            <table border="0" cellpadding="0" cellspacing="0" class="store-joinin">
        <thead>
          <tr>
            <th colspan="20">纳税识别号</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <th class="w150">纳税识别号：</th>
            <td><input type="text" class="txt w300" name="taxpayer_id" value="'.$output['joinin_detail']['taxpayer_id'].'"></td>
          </tr>
        </tbody>
      </table>
            <table border="0" cellpadding="0" cellspacing="0" class="store-joinin">
            <thead>
            <tr>
                <th colspan="20">发票抬头</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <th class="w150">发票抬头：</th>
                <td><input type="text" class="txt w300" name="invoice_title"
                           value="'.$output['joinin_detail']['invoice_title'].'"></td>
            </tr>
            </tbody>
        </table>';
        }
...
<?php }else { ?>
  <form id="joinin_form" enctype="multipart/form-data" method="post" action="index.php?act=store&op=edit_save_joinin" style="display:none;">
    ...
    <tbody>
	  ...
	  <th>身份证正面<br/>          电子版：</th>
        <td colspan="20">
          <a nctype="nyroModal"  href="<?php echo getStoreJoininImageUrl($output['joinin_detail']['business_licence_number_electronic']);?>"> <img src="<?php echo getStoreJoininImageUrl($output['joinin_detail']['enclosure_electronic']);?>" alt="" /> </a>
          <input class="w200" type="file" name="enclosure_electronic">
        </td>
	 </tr>
      <tr>
        <th>身份证反面<br/>          电子版：</th>
    ...
    $('#store_form').validate({
        ...
        messages : {
            store_name: {
                ...
                remote   : '<i class="fa fa-exclamation-circle"></i>供应商名称已存在'
            }
        }
    });
