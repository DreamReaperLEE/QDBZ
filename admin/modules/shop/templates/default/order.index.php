            ...
            <div id="searchCon" class="content">
                <div class="layout-box">
                    <dl>
                        ...
                        <dd>
                            <label>
                                <select class="s-select" name="keyword_type" onchange = enterpriseProjectEvent()>
                                    <option selected="selected" value="">-请选择-</option>
                                    <option value="order_sn">订单编号</option>
                                    <option value="buyer_name">供应商</option>
                                    <option value="store_name">店铺名称</option>
                                    <option value="pay_sn">支付单号</option>
                                    <option value="seller_name">保障单位</option>
                                    <option value="shipping_code">发货单号</option>
                                    <!--                    <option value="project_name">项目名称</option>-->
                                </select>
                            </label>
                            <label>
                                <input type="text" value="" placeholder="请输入关键字" name="keyword" id="keyword" class="s-input-txt" onchange="keywordchange()">
                            </label>

                        <dt id="projectdt" style="display: none;">项目名称</dt>
                        <dd>
                            <select class="s-select" name="project_name"  id= "project_name" style="display:none;"  >
                                <option selected="selected" value="">-请选择-</option>
                            </select>
...
<script type="text/javascript">
    $(function(){
        ...
        $("#flexigrid").flexigrid({
            ...
            colModel : [
                ...
                {display: '保障单位', name : 'seller_name', width : 150, sortable : true, align: 'left'},
                {display: '项目名称', name : 'project_name', width : 150, sortable : true, align: 'left'},
                ...
                {display: '下单人', name : 'buyer_name', width : 150, sortable : true, align: 'left'}
            ],
            buttons : [
            ...
            searchitems : [
                ...
                {display: '供应商', name : 'buyer_name'},
                {display: '店铺名称', name : 'store_name'},
                {display: '支付单号', name : 'pay_sn'},
                {display: '保障单位', name : 'seller_name'}
            ],
            sortname: "order_id",
            sortorder: "desc",
            title: '线上交易实物订单明细'
    ...
    function enterpriseProjectEvent(){
        if(document.getElementsByName("keyword_type")[0].selectedIndex == 5)
        {
            document.getElementById("projectdt").style.display = "block";
            document.getElementsByName("project_name")[0].style.display="block";
        }
        else
        {
            document.getElementById("projectdt").style.display = "none";
            document.getElementsByName("project_name")[0].style.display="none";
        }
    }


    function keywordchange() {
        if(document.getElementsByName("keyword_type")[0].selectedIndex == 5){//选择的是企业账号，并且当关键字文本框改变时
            var textValue = document.getElementById('keyword').value;
            $.ajax({
                type: "POST",
                dataType: "json",
                url: "index.php?act=order&op=getProject",
                data: { textValue:textValue },
                success:function (data) {
                    var json = eval(data);
                    $('#project_name option').not(":first").remove();
                    $.each(json, function (index, item) {
                        //循环获取数据    
                        var address = json[index].address;
                        $("#project_name").append("<option value='"+address+"'>"+address+"</option>");
                    });
                },
                error: function () {
                    console.log("error");
