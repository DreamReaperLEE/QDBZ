...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?act=company_class" title="返回列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
          <h3>保障单位分类 编辑</h3>
          <h5>保障中心保障单位分类管理设置</h5>
      </div>
    </div>
  </div>
	...
	$('#store_class_form').validate({
        ...
        rules : {
            sc_name : {
                required : true,
                remote   : {
                url :'index.php?act=company_class&op=ajax_check_name',
                type:'get',
                data:{
                    sc_name : function(){
                        return $('#sc_name').val();
