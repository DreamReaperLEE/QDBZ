...
class storeControl extends SystemControl{
    ...
    private $_links = array(
        array('url'=>'act=store&op=store&is_company=0','text'=>'管理'),
	...
	if ($_GET['is_company'] != '') {
            $condition['is_company'] = $_GET['is_company'];
        }
        if ($_POST['query'] != '') {
        ...
        foreach ($store_list as $value) {
            ...
            $operation = "<a class='btn green' href='index.php?act=store&op=store_joinin_detail&member_id=".$value['member_id']."'><i class='fa fa-list-alt'></i>查看</a><span class='btn'><em><i class='fa fa-cog'></i>" . L('nc_set') . " <i class='arrow'></i></em><ul><li><a href='index.php?act=store&op=store_edit&store_id=" . $value['store_id'] . "+&is_company=" . $value['is_company'] . "'>编辑信息</a></li><li><a href='index.php?act=store&op=del&store_id=" . $value['store_id'] . "'>删除信息</a></li><li><a href='index.php?act=store&op=store_bind_class&store_id=" . $value['store_id'] . "'>修改经营类目</a></li>";
    ...
    public function edit_save_joininOp() {
        if (chksubmit()) {
            ...
            if ($_FILES['enclosure_electronic']['name'] != '') {
                $param['enclosure_electronic'] = $this->upload_image('enclosure_electronic');
            }
    ...
    private function store_joinin_verify_open($joinin_detail) {
        ...
        if($_POST['verify_type'] === 'pass') {
            ...
            $shop_array['is_company'] = $joinin_detail['is_company'];
            ...
            if($store_id) {
                ...
                $seller_array['is_company'] = $joinin_detail['is_company'];
