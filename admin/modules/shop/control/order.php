...
class orderControl extends SystemControl{
    ...
    //根据企业账号获取项目名称
    public function getProjectOp(){
        unset($commonlist);
        $textValue['seller_name'] = $_POST['textValue'];//获取关键字（企业账号）
        $model_seller = Model('seller');
        $model_address= Model('address');
        $result= $model_seller->getSellerInfo($textValue);//读取sell的单条记录
        $condition['member_id'] =$result['member_id'];//sell表里的member_id
        $commonlist = $model_address->getAddressList($condition);
        json_encode($commonlist);
...
//            unset($condition['seller_name']);
//            $model_order1 = Model('order');
//            $condition3=array();
//            $project_list = $model_order1->getOrderList($condition3,'','','',1000);
//            $project_list1 = array();
//            foreach ($project_list as $every) {
//                $order_info = $model_order1->getOrderInfo(array('order_id'=>$every['order_id']),array('order_common'));
//                if($condition['project_name1']!=''){
//                    if(strpos($order_info['extend_order_common']['reciver_info']['street'],$condition['project_name1'])!==false){
//                        $project_list1[] = (int)$every['order_id'];
//                    }
//                }
//                if($condition['project_name']==$order_info['extend_order_common']['reciver_info']['street']){
//                    $project_list1[] = (int)$every['order_id'];
//                }
//            }
//            $condition['order_id'] = array('in', $project_list1);
//            unset($condition['project_name']);
//            unset($condition['project_name1']);
//        }

        $condition1  = array();
        $this->_get_condition($condition1);
        //按企业账号查询条件
        if($condition1['seller_name']!=''){
            $seller1 = Model('seller');
            unset($condition1['project_name']);
            unset($condition1['project_name1']);
            $sstoreid=$seller1->getisadmin($condition1);
            $sstoreid1=array();
            foreach($sstoreid as $every){
                $sisadmin=(int)$every['is_admin'];
                if($sisadmin==1){
                    $sstoreid1[]=(int)$every['store_id'];//获取is_admin为1的store_id存入数组$sstoreid1
                }
            }
            $condition2 = array();
            $condition2['store_id'] = array('in',$sstoreid1);
            $seller_list = $seller1->getMember($condition2);
            $seller_list1 = [];
            foreach ($seller_list as $every) {
                $seller_list1[] = (int)$every['member_id'];
            }
            //$condition1['buyer_id'] = array('in', $seller_list1);//该企业账号下的buyer_id
            unset($condition1['seller_name']);
            ...
            if($condition['project_name']!=''){
                ...
                $project_list1 = array();
                foreach ($project_list as $every) {
                    if(in_array($every['buyer_id'],$seller_list1) ){
                        $order_info = $model_order1->getOrderInfo(array('order_id'=>$every['order_id']),array('order_common'));
                        if($condition['project_name1']!=''){
                            if(strpos($order_info['extend_order_common']['reciver_info']['street'],$condition['project_name1'])!==false){
                                $project_list1[] = (int)$every['order_id'];
...
//            $condition  = array();
//            $this->_get_condition($condition);
//            $condition = $this->_get_condition($condition);
//            if($condition['project_name']!=''){
//                unset($condition['seller_name']);
//                $model_order1 = Model('order');
//                $condition3=array();
//                $project_list2 = $model_order1->getOrderList($condition3,'','','',1000);
//                $project_list = array();
//                foreach ($project_list2 as $every){
//                    if($every['buyer_id'] == $condition1['buyer_id'])
//                        $project_list[] = $every;
//                }
//                $project_list1 = array();
//                foreach ($project_list as $every) {
...
//                $sisadmin=(int)$every['is_admin'];
//                if($sisadmin==1){
//                    $sstoreid1[]=(int)$every['store_id'];//获取is_admin为1的store_id
//                }
//            }
//            $condition2 = array();
//            $condition2['store_id'] = array('in',$sstoreid1);
...
//        }
        ...
        $sort_fields = array('buyer_name','store_name','order_id','payment_code','order_state','order_amount','order_from','pay_sn','rcb_amount','pd_amount','payment_time','finnshed_time','evaluation_state','refund_amount','buyer_id','store_id','seller_name','project_name');
        ...
        foreach ($order_list as $order_id => $order_info) {
            ...
            $order_info1 = $model_order->getOrderInfo(array('order_id'=>$order_info['order_id']),array('order_common'));
            $order_info['project_name'] =$order_info1['extend_order_common']['reciver_info']['street'];

            //显示企业账号
            $seller = Model('seller');
            $condition0=array();
            $condition0['member_id']=$order_info['buyer_id'];
            $storeid=$seller->getisadmin($condition0);
            foreach($storeid as $every0){
                $storeid0=(int)$every0['store_id'];
            }

            $condition1=array();
            $condition1['store_id']=$storeid0;
            $condition1['is_admin']=1;
            $sellername=$seller->getisadmin($condition1);
            foreach($sellername as $every){
                $sellername1=$every['seller_name'];
            }

            $order_info['seller_name']=$sellername1;
            ...
            $list['seller_name'] = $order_info['seller_name'];
            $list['project_name'] = $order_info['project_name'];
    ...
    private function _get_condition(& $condition) {
        if ($_REQUEST['query'] != '' && in_array($_REQUEST['qtype'],array('order_sn','store_name','buyer_name','pay_sn','seller_name','project_name'))) {
            $condition[$_REQUEST['qtype']] = array('like',"%{$_REQUEST['query']}%");
        }

        if ($_GET['keyword'] != '' && in_array($_GET['keyword_type'],array('order_sn','store_name','buyer_name','pay_sn','seller_name','shipping_code'))) {
            if ($_GET['jq_query']) {
                if (in_array($_GET['keyword_type'], array('order_sn', 'store_name', 'buyer_name', 'pay_sn', 'shipping_code'))) {
                    $condition[$_GET['keyword_type']] = $_GET['keyword'];
                } else if (in_array($_GET['keyword_type'], array('seller_name'))) {
                    unset($condition['seller_name']);
                    $condition['seller_name'] = $_GET['keyword'];
                    $condition['project_name'] = $_GET['project_name'];
                ...
                if (in_array($_GET['keyword_type'], array('order_sn', 'store_name', 'buyer_name', 'pay_sn', 'shipping_code'))) {
                    ...
                    unset($condition['seller_name']);
                    $condition['seller_name'] = array('like',"%{$_GET['keyword']}%");
                    $condition['project_name'] = $_GET['project_name'];
                    $condition['project_name1'] = $_GET['project_name'];
