...
class store_joinin2Control extends BaseHomeControl {
    ...
    public function __construct() {
        ...
        Tpl::output('html_title',C('site_name').' - '.'保障单位入驻');
            ...
            switch (intval($joinin_detail['joinin_state'])) {
                ...
                case STORE_JOIN_STATE_VERIFY_FAIL:
                    if(!in_array($_GET['op'], array('step1', 'step2', 'step3', 'step4','ajax_upload_image'))) {
                        $this->show_join_message('审核失败:'.$joinin_detail['joinin_message'], SHOP_SITE_URL.DS.'index.php?act=store_joinin2&op=step1');
                    }
                    break;
                case STORE_JOIN_STATE_PAY_FAIL:
                    if(!in_array($_GET['op'], array('pay', 'pay_save'))) {
                        $this->show_join_message('付款审核失败:'.$joinin_detail['joinin_message'], SHOP_SITE_URL.DS.'index.php?act=store_joinin2&op=pay');
                    }
                    break;
    ...
    public function step0Op() {
        ...
        Tpl::showpage('store_joinin2_apply');
        exit;
    }
    ...
    public function step2Op() {
        if(!empty($_POST)) {
            ...
            $param['is_company'] = 1;
            $param['enclosure_name'] = $_POST['enclosure_name'];
            $param['enclosure_electronic'] = $_POST['enclosure_electronic1'];
    ...
    private function step2_save_valid($param) {
        ...
        $obj_validate->validateparam = array(
            ...
            //array("input"=>$param['enclosure_name'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"20","message"=>"组织机构代码不能为空且必须小于20个字"),
            //array("input"=>$param['enclosure_electronic'], "require"=>"true","message"=>"组织机构代码电子版不能为空"),
        );
...
//            $param['tax_registration_certif_elc'] = $_POST['tax_registration_certif_elc1'];
        ...
        $model_store_grade_2 = Model('store_grade_2');
        $grade_list = $model_store_grade_2->getGradeList();
        //店铺等级
        //$grade_list = rkcache('store_grade_2',true);
        //附加功能
        ...
        //店铺分类
        $model_store = Model('store_class_2');
...
//            array("input"=>$param['tax_registration_certif_elc'], "require"=>"true","message"=>"税务登记证号电子版不能为空"),
    ...
    public function step4Op() {
        ...
        $param['invoice_title'] = $_POST['invoice_title'];
...
//        $model_store_grade = Model('store_grade_2');
//        //取店铺等级信息
         $grade_list = rkcache('store_grade2',true);
      //$grade_list = $model_store_grade->getGradeList();
        ...
        //取最新店铺分类信息
        $store_class_info = Model('store_class_2')->getStoreClassInfo(array('sc_id'=>intval($_POST['sc_id'])));
        if ($store_class_info) {
        ...
        @header('location: index.php?act=store_joinin2');
    ...
    public function payOp() {
        if (!empty($this->joinin_detail['sg_info'])) {
            $store_grade_info = Model('store_grade_2')->getOneGrade($this->joinin_detail['sg_id']);
