<?php
/**
 * 前台control父类,供应商control父类,会员control父类
 *
 *
    ...
    private function _getMenuList() {
        $menu_list = array(
            ...
            'follow' => array('name' => '关注中心', 'child' => array(
                ...
                'member_favorite_store' => array('name' => '供应商收藏', 'url' => urlShop('member_favorite_store', 'index')),
            ...
            'info' => array('name' => '会员资料', 'child' => array(
                ...
                'member_address'    => array('name' => '项目管理', 'url'=>urlMember('member_address', 'address'))
            )),
...
//                array('name' => '查看员工订单', 'act'=>'company_order', 'op'=>'index'),
//                array('name' => '服务订单统计', 'act'=>'company_calculate', 'op'=>'gcidSpend'),
//                array('name' => '项目订单统计', 'act'=>'company_calculate2', 'op'=>'companySpend2'),
            )),
...
//            )),
            'store' => array('name' => '供应商', 'child' => array(
                array('name' => '供应商设置', 'act'=>'store_setting', 'op'=>'store_setting'),
                array('name' => '供应商装修', 'act'=>'store_decoration', 'op'=>'decoration_setting'),
                array('name' => '供应商导航', 'act'=>'store_navigation', 'op'=>'navigation_list'),
                array('name' => '供应商动态', 'act'=>'store_sns', 'op'=>'index'),
                array('name' => '供应商信息', 'act'=>'store_info', 'op'=>'bind_class'),
                array('name' => '供应商分类', 'act'=>'store_goods_class', 'op'=>'index'),
                array('name' => '品牌申请', 'act'=>'store_brand', 'op'=>'brand_list'),
                array('name' => '供货商', 'act'=>'store_supplier', 'op'=>'sup_list'),
                array('name' => '实体供应商', 'act'=>'store_map', 'op'=>'index'),
            ...
            'statistics' => array('name' => '统计结算', 'child' => array(
                array('name' => '供应商概况', 'act'=>'statistics_general', 'op'=>'general'),
...
//            'account' => array('name' => '保障单位', 'child' => array(
//                array('name' => '员工列表', 'act'=>'store_account', 'op'=>'account_list'),
//                array('name' => '角色组', 'act'=>'store_account_group', 'op'=>'group_list'),
//                array('name' => '账号日志', 'act'=>'seller_log', 'op'=>'log_list'),
//                array('name' => '供应商消费', 'act'=>'store_cost', 'op'=>'cost_list'),
////                array('name' => '门店账号', 'act'=>'store_chain', 'op'=>'index'),
//                array('name' => '项目管理', 'act'=>'store_account_group2', 'op'=>'project_show')
//            ))
			//临时注释
...
class BaseCompanyControl extends Control {
    ...
    public function __construct(){
        ...
        if(!C('site_status')) halt(C('closed_reason'));
        Tpl::setDir('seller');
        Tpl::setLayout('company_layout');

        Tpl::output('nav_list', rkcache('nav',true));
        if ($_GET['act'] !== 'company_login') {

            if(empty($_SESSION['seller_id'])) {
                @header('location: index.php?act=company_login&op=show_login');die;
            }
            ...
            if($_GET['act'] == 'seller_center') {
                if(!empty($_SESSION['seller_quicklink'])) {
                    $left_menu2 = array();
                    foreach ($_SESSION['seller_quicklink'] as $value) {
                        $left_menu2[] = $_SESSION['seller_function_list'][$value];
                    }
                }
            } else {
                $left_menu2 = $_SESSION['seller_menu'][$current_menu['model']]['child'];
            }
            Tpl::output('left_menu', $left_menu2);
            Tpl::output('seller_quicklink', $_SESSION['seller_quicklink']);
    ...
    protected function getSellerMenuList2($is_admin, $limits) {
        $seller_menu = array();
        if (intval($is_admin) !== 1) {
            $menu_list = $this->_getMenuList2();
            foreach ($menu_list as $key => $value) {
                foreach ($value['child'] as $child_key => $child_value) {
                    if (!in_array($child_value['act'], $limits)) {
                        unset($menu_list[$key]['child'][$child_key]);
            ...
            $seller_menu = $this->_getMenuList2();
        }
        $seller_function_list = $this->_getSellerFunctionList($seller_menu);
        return array('seller_menu' => $seller_menu, 'seller_function_list' => $seller_function_list);
    }
    ...
    protected function _getMenuList2() {

        $menu_list2 = array(
//            'goods' => array('name' => '商品123', 'child' => array(
//                array('name' => '商品发布', 'act'=>'store_goods_add', 'op'=>'index'),
//                array('name' => '淘宝CSV导入', 'act'=>'taobao_import', 'op'=>'index'),
//                array('name' => '出售中的商品', 'act'=>'store_goods_online', 'op'=>'index'),
//                array('name' => '仓库中的商品', 'act'=>'store_goods_offline', 'op'=>'index'),
