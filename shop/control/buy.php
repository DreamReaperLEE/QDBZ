...
class buyControl extends BaseBuyControl {
    ...
    public function buy_step1Op() {
        ...
        Tpl::output('spec_r', $_POST['spec_r']);

        Tpl::output('begindate',$_POST['begindate']);

        Tpl::output('outdate',$_POST['enddate']);
    ...
    public function payOp() {
        ...
        $condition0['pay_sn']= $pay_sn;
        $buyerid=$model_order0->getBuyerid($condition0);

        foreach($buyerid as $every0){
            $buyerid0=(int)$every0['buyer_id'];
        }
//                Tpl::output('seller_list', json_encode($memberid0));
//        Tpl::showpage('member_order.index');
        $pay_info = $model_order->getOrderPayInfo(array('pay_sn'=>$pay_sn,'buyer_id'=>$buyerid0),true);
        if(empty($pay_info)){
            showMessage(Language::get('cart_order_pay_not_exists'),'index.php?act=member_order','html','error');
        }
    ...
    public function pay_okOp() {
        ...
        $model_order= Model('order');
        $pay_info = $model_order->getOrderPayInfo(array('pay_sn'=>$pay_sn,'buyer_id'=>$buyerid0));
        if(empty($pay_info)){
            showMessage(Language::get('cart_order_pay_not_exists'),'index.php?act=member_order','html','error');
        }
        Tpl::output('pay_info',$pay_info);
    ...
    public function load_addrOp() {
        ...
        $model_addr_member = Model('address_member');

        $condition['member_id'] = $_SESSION['member_id'];

        $address_id_list = $model_addr_member->getaddress_member_list($condition);
        foreach($address_id_list as $every) {
        ...
        $condition2['address_id'] = 21;
        $def = $model_addr->getAddressInfo($condition2);
        array_push($list,$def);
