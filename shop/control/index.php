<?php
/**
 * 默认展示页面
 *
 *
 *
 * @好商城提供技术支持 授权请购买shopnc授权
 * @license    http://www.33hao.com
 * @link       交流群号：138182377
 */


defined('In33hao') or exit('Access Invalid!');
class indexControl extends BaseHomeControl{
    public function indexOp(){
        Language::read('home_index_index');
        Tpl::output('index_sign','index');

	//把加密的用户id写入cookie  by 3 3h ao.co m 已换另一个方式，临时去掉此方法
	$uid = intval(base64_decode($_COOKIE['uid']));
//        //抢购专区
//        Language::read('member_groupbuy');
//        $model_groupbuy = Model('groupbuy');
//        $group_list = $model_groupbuy->getGroupbuyCommendedList(4);
//        Tpl::output('group_list', $group_list);
//
//		//专题获取
//
//        $model_special = Model('cms_special');
//        $special_list = $model_special->getShopindexList($conition);
//        Tpl::output('special_list', $special_list);
	
	//友情链接
//	$model_link = Model('link');
//	$link_list = $model_link->getLinkList($condition,$page);
//	if (is_array($link_list)){
//		foreach ($link_list as $k => $v){
//			if (!empty($v['link_pic'])){
//				$link_list[$k]['link_pic'] = UPLOAD_SITE_URL.'/'.ATTACH_PATH.'/common/'.DS.$v['link_pic'];
//			}
//		}
//	}
//	Tpl::output('$link_list',$link_list);
		
        //限时折扣
//        $model_xianshi_goods = Model('p_xianshi_goods');
//        $xianshi_item = $model_xianshi_goods->getXianshiGoodsCommendList(6);
//        Tpl::output('xianshi_item', $xianshi_item);
		
		//直达楼层信息
//		 if (C('hao_lc') != '') {
//            $lc_list = @unserialize(C('hao_lc'));
//        }
//        Tpl::output('lc_list',is_array($lc_list) ? $lc_list : array());
		
		//首页推荐词链接
//		 if (C('hao_rc') != '') {
//            $rc_list = @unserialize(C('hao_rc'));
//        }
//        Tpl::output('rc_list',is_array($rc_list) ? $rc_list : array());

        //推荐品牌
//        $brand_r_list = Model('brand')->getBrandPassedList(array('brand_recommend'=>1) ,'brand_id,brand_name,brand_pic,brand_xbgpic,brand_tjstore', 0, 'brand_sort asc, brand_id desc', 16);
//        Tpl::output('brand_r',$brand_r_list);
//
		
		//评价信息
//        $goods_evaluate_info = Model('evaluate_goods')->getEvaluateGoodsList(8);
//        Tpl::output('goods_evaluate_info', $goods_evaluate_info);
	
	//当前城市 v5.3
//		$city_id = cookie('cityid') ? cookie('cityid') : 0;
//		if ($city_id != 0) {
//			Tpl::output('now_city',Model('area')->getAreaInfoById($city_id));
//		}else{
//			Tpl::output('now_city','全国');
//		}
        //一级导航
        $class_top=Model('goods_class')->getGoodsClassListByParentId(0);
        Tpl::output('class_top',is_array($class_top) ? $class_top : array());
        //日常保障
        $class_bao=Model('goods_class')->getGoodsClassListByParentId($class_top[0]['gc_id']);
        Tpl::output('class_bao',is_array($class_bao) ? $class_bao : array());
        //技术装备
        $class_jisu=Model('goods_class')->getGoodsClassListByParentId($class_top[1]['gc_id']);
        if(!empty($class_jisu)) {
            foreach ($class_jisu as $k => $v) {
                $class_jisu[$k] = $v;
                $class_jisu[$k]['goods'] = Model('goods')->getGoodsOnlineList(array('gc_id_2' => $v['gc_id']), 'goods_id,goods_name,goods_addtime', '', 'goods_id desc', 3);
            }
        }
        Tpl::output('class_jisu',is_array($class_jisu) ? $class_jisu : array());
        //物资采购
        $class_gou=Model('goods_class')->getGoodsClassListByParentId($class_top[2]['gc_id']);
        if(!empty($class_gou)) {
            foreach ($class_gou as $k => $v) {
                $class_gou[$k] = $v;
                $class_gou[$k]['goods'] = Model('goods')->getGoodsOnlineList(array('gc_id_2' => $v['gc_id']), 'goods_id,goods_name,goods_addtime', '', 'goods_id desc', 6);
            }
        }
        Tpl::output('class_gou',is_array($class_gou) ? $class_gou : array());
        //教育培训
        $class_teach=Model('goods')->getGoodsOnlineList(array('gc_id_1'=>$class_top[3]['gc_id']), 'goods_id,goods_name,goods_addtime,goods_jingle,goods_image', '', 'goods_id desc');
        $k=0;
        if(!empty($class_teach)){
            foreach ($class_teach as $key=>$v){
                if($key%3==0&&$key>=3)$k++;
                $class_teachs[$k][]=$v;
            }
        }
        Tpl::output('class_teachs',is_array($class_teachs) ? array_reverse($class_teachs) : array());
        Tpl::output('k',$k);
        //通知公告
        $condition['ac_id']=8;
        $condition['limit']=6;
        $condition['order']='article_time desc';
        $notice=Model('article')->getArticleList($condition);
        /**
         * 模型实例化
         */
        $model_upload = Model('upload');
        $cond['upload_type'] = '1';
        if(!empty($notice)) {
            foreach ($notice as $k => $v) {
                $cond['item_id'] = $v['article_id'];
                $fields = $model_upload->getUploadList($cond);
                $field = is_array($fields) ? $fields[0]['file_name'] : $fields['file_name'];
                $notice[$k]['upload_path'] = UPLOAD_SITE_URL . '/' . ATTACH_ARTICLE . '/' . $field;
            }
        }
        Tpl::output('notice',is_array($notice) ? $notice : array());
        //统计
        $stat_model = Model('stat');
        //保障单位
        $where = array();
        $where['member_time'] = array('gt',0);
        $field = ' COUNT(*) as membernum ';
        $stat_member = $stat_model->getoneByMember($where, $field);
        $numbers['conpany']=($t = $stat_member['membernum'])?$t:0;
        unset($stat_member);
        //供应商
        $where = array();
        $where['store_time'] = array('gt',0);
        $field = ' COUNT(*) as storenum ';
        $stat_store = $stat_model->getoneByStore($where, $field);
        $numbers['store']=($t = $stat_store['storenum'])?$t:0;
        unset($stat_store);
        //服务数量
        $numbers['goods']=Model('goods')->getGoodsCommonCount(1);
        //成交额
        $where = array();
        $where['order_isvalid'] = 1;//计入统计的有效订单
        $field = 'SUM(order_amount) as orderamount) as orderavg ';
        $stat_order = $stat_model->getoneByStatorder($where, $field);
        $numbers['orderamount'] = ncPriceFormat(($t = $stat_order['orderamount'])?$t:(0));
        Tpl::output('numbers',$numbers);
        //板块信息
        $model_web_config = Model('web_config');
        $web_html = $model_web_config->getWebHtml('index');
        $imgpreg = "/url\(\'(.+?)\'.*?>/";
        preg_match_all($imgpreg,$web_html['index_pic'],$img);

        Tpl::output('web_img',$img[1]);
        Tpl::output('is_index',1);
        Model('seo')->type('index')->show();
        Tpl::showpage('index');
    }
    
    //选择城市 v5.3
	public function set_cityOp() {
		$city_id = intval($_GET['cityid']);
		if ($city_id != 0) {
			$city_info = Model('area')->getAreaInfoById($city_id);
			if (empty($city_info)) {
				showMessage('该城市不存在，请选择其他城市');
			}
	        
		}
		setNcCookie('cityid', $city_id);
		$ref_url=$_GET['ref_url'];
		$isExist = strstr($ref_url, 'act=select_city&');
		if($ref_url==''||$isExist)
		{
			header('Location:'.BASE_SITE_URL);
			return;
		}
        header('Location:'.$ref_url);
	}
	//选择城市的页面 v5.3
	public function select_cityOp() {
		$area_list = Model('area')->getAreaArrayForJson();
		//省份数据
		$province = $area_list[0];
		foreach ($province as $key => $value) {
			//市的数据
			$city_list[][$key] = $area_list[$value[0]];
		}
		Tpl::output('province_list',$province);
		Tpl::output('city_list',$city_list);
		Tpl::showpage('select_city');
	}

    //json输出商品分类
    public function josn_classOp() {
        /**
         * 实例化商品分类模型
         */
        $model_class        = Model('goods_class');
        $goods_class        = $model_class->getGoodsClassListByParentId(intval($_GET['gc_id']));
        $array              = array();
        if(is_array($goods_class) and count($goods_class)>0) {
            foreach ($goods_class as $val) {
                $array[$val['gc_id']] = array('gc_id'=>$val['gc_id'],'gc_name'=>htmlspecialchars($val['gc_name']),'gc_parent_id'=>$val['gc_parent_id'],'commis_rate'=>$val['commis_rate'],'gc_sort'=>$val['gc_sort']);
            }
        }
        /**
         * 转码
         */
        if (strtoupper(CHARSET) == 'GBK'){
            $array = Language::getUTF8(array_values($array));//网站GBK使用编码时,转换为UTF-8,防止json输出汉字问题
        } else {
            $array = array_values($array);
        }
        echo $_GET['callback'].'('.json_encode($array).')';
    }

    /**
     * json输出地址数组 原data/resource/js/area_array.js
     */
    public function json_areaOp()
    {
        $_GET['src'] = $_GET['src'] != 'db' ? 'cache' : 'db';
        echo $_GET['callback'].'('.json_encode(Model('area')->getAreaArrayForJson($_GET['src'])).')';
    }

    /**
     * 根据ID返回所有父级地区名称
     */
    public function json_area_showOp()
    {
        $area_info['text'] = Model('area')->getTopAreaName(intval($_GET['area_id']));
        echo $_GET['callback'].'('.json_encode($area_info).')';
    }

    //判断是否登录
    public function loginOp(){
        echo ($_SESSION['is_login'] == '1')? '1':'0';
    }

    /**
     * 头部最近浏览的商品
     */
    public function viewed_infoOp(){
        $info = array();
        if ($_SESSION['is_login'] == '1') {
            $member_id = $_SESSION['member_id'];
            $info['m_id'] = $member_id;
            if (C('voucher_allow') == 1) {
                $time_to = time();//当前日期
                $info['voucher'] = Model()->table('voucher')->where(array('voucher_owner_id'=> $member_id,'voucher_state'=> 1,
                'voucher_start_date'=> array('elt',$time_to),'voucher_end_date'=> array('egt',$time_to)))->count();
            }
            $time_to = strtotime(date('Y-m-d'));//当前日期
            $time_from = date('Y-m-d',($time_to-60*60*24*7));//7天前
            $info['consult'] = Model()->table('consult')->where(array('member_id'=> $member_id,
            'consult_reply_time'=> array(array('gt',strtotime($time_from)),array('lt',$time_to+60*60*24),'and')))->count();
        }
        $goods_list = Model('goods_browse')->getViewedGoodsList($_SESSION['member_id'],5);
        if(is_array($goods_list) && !empty($goods_list)) {
            $viewed_goods = array();
            foreach ($goods_list as $key => $val) {
                $goods_id = $val['goods_id'];
                $val['url'] = urlShop('goods', 'index', array('goods_id' => $goods_id));
                $val['goods_image'] = thumb($val, 60);
                $viewed_goods[$goods_id] = $val;
            }
            $info['viewed_goods'] = $viewed_goods;
        }
        if (strtoupper(CHARSET) == 'GBK'){
            $info = Language::getUTF8($info);
        }
        echo json_encode($info);
    }
    /**
     * 查询每月的周数组
     */
    public function getweekofmonthOp(){
        import('function.datehelper');
        $year = $_GET['y'];
        $month = $_GET['m'];
        $week_arr = getMonthWeekArr($year, $month);
        echo json_encode($week_arr);
        die;
    }
}
