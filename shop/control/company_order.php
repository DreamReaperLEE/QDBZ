...
class company_orderControl extends BaseCompanyControl {
    ...
    public function indexOp() {
        ...
        $isadmin=$model_seller->getisadmin($condition0);
        foreach($isadmin as $every0){
            $isadmin1=(int)$every0['is_admin'];
            $isadmin2=(int)$every0['store_id'];
        }
        if ($isadmin1==1) {

            $condition1 = array();
            $condition1['store_id'] =$isadmin2;
            $seller_list=$model_seller->getMember($condition1);
...
//        $_SESSION['member_id']
        ...
        Tpl::showpage('companyorder.list');
    }
    ...
    public function search_deliverOp(){
        ...
        $model_order0 = Model('order');
        $condition0['order_id']= $order_id;
        $memberid=$model_order0->getmember($condition0);

        foreach($memberid as $every0){
            $memberid0=(int)$every0['buyer_id'];
        }
        if ($order_id <= 0) {
    ...
    public function show_orderOp() {
        ...
        $model_order0 = Model('order');
        $condition0['order_id']= intval($_GET['order_id']);
        $memberid=$model_order0->getmember($condition0);

        foreach($memberid as $every0){
            $memberid0=(int)$every0['buyer_id'];
        }
        $result = $logic_order->getMemberOrderInfo($_GET['order_id'],$memberid0);
        if (!$result['state']) {
    ...
    public function change_stateOp() {
        ...
        if ($isadmin1==1){
            $condition0['order_id']= $order_id;
             ...
             $condition0['order_id']= $_SESSION['member_id'];
         }


        $memberid=$model_order0->getmember($condition0);
        $memberid1=array();
        foreach($memberid as $every){
            $memberid1=(int)$every['buyer_id'];
        }
//        Tpl::output('seller_list', json_encode($memberid1));
//        Tpl::showpage('member_order.index');
        $condition = array();
        $condition['order_id'] = $order_id;
        $condition['buyer_id'] = $memberid1;
