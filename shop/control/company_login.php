...
class company_loginControl extends BaseCompanyControl
{
    ...
    public function __construct()
    {
        ...
        if (!empty($_SESSION['seller_id'])) {
            @header('location: index.php?act=company_center');
            die;
        }
    }
    ...
    public function loginOp()
    {
        ...
        if ($seller_info['is_company'] == 1) {
                ...
                if ($member_info) {
                    ...
                    $_SESSION['hello'] = $seller_group_info['limits'];
                    ...
                    if ($seller_info['is_company']) {
                        $_SESSION['seller_group_name'] = '企业用户';
                        $is_company = 1;
                        $_SESSION['seller_smt_limits'] = false;
                    } elseif ($seller_info['is_admin']) {
                        $_SESSION['seller_group_name'] = '管理员';
                        $_SESSION['seller_smt_limits'] = false;
                    } elseif ($seller_info['is_client']) {
                    ...
                    $seller_menu = $this->getSellerMenuList2($seller_info['is_admin'], explode(',', $seller_group_info['limits']));
                    ...
                    redirect('index.php?act=company_center');
