...
<div class="ncsl-nav">
    ...
    <div id="nav" class="ncs-nav">
	  ...
	  <?php if(!empty($output['goods_class_list'])){?>
        ...
        <?php }}}} ?>
        <li class="<?php if ($output['page'] == 'store_sns') {?>active<?php }else{?>normal<?php }?>"><a href="<?php echo urlShop('store_snshome', 'index', array('sid' => $output['store_info']['store_id']))?>"><span>供应商动态<i></i></span></a></li>
      </ul>
    </div>
  </div>
</div>
