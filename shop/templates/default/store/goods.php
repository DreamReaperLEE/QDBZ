...
<link href="<?php echo SHOP_TEMPLATES_URL; ?>/bootstrap-datetimepicker-master/css/bootstrap-datetimepicker.css" rel="stylesheet" media="screen">
<script type="text/javascript" src="<?php echo SHOP_TEMPLATES_URL; ?>/bootstrap-datetimepicker-master/js/bootstrap-datetimepicker.js"></script>
        ...
        <?php if ($output['goods']['gc_id_1'] == '1') {?>
            ...
            <dl class="good-price">
                ...
                <dd class="price">
                    ...
                    <?php } else {?>
                        <strong id="price_goods"><?php echo $lang['currency'].ncPriceFormat($output['goods']['goods_price']);?></strong>
                    <?php }?>
                </dd>
            </dl>
        <div style="text-align: center; padding: 20px;">
        <span>入住日期：</span><input type="date" id="date_start" onchange="getHotelPrice()" style="width:136px;margin-top:-3px;color:#666;border:0;outline:0;background-color:transparent;font-size:14px;vertical-align:middle"/><br/><br/>
...
<!--          <p>三号位</p>-->
        <?php } else if ($output['goods']['gc_id_1'] == '256') {?>
          <style type="text/css">body, html {width: 100%;height: 100%; margin:0;font-family:"微软雅黑";}
...
<br />
<span id = "tooltip">总路程为：</span><span id = "tooltip_num">_______</span><span id = "tooltip_after">公里</span>
	<!-- <div id="allmap"></div> -->
	<div id="driving_way">
  ...
  if( start == "" || end == "" ) {
      ...
      return;
		...
		output = plan.getDistance(true).slice(0,this.length-3) + "\n";             //获取距离
            ...
            var driving = new BMap.DrivingRoute(map, {
                renderOptions: {map: map, autoViewport: true}, policy: route, onSearchComplete: searchComplete,
    ...
    var output = "";
        ...
        <dl class="good-price">
          ...
          <dd class="price">
            ...
            <strong>
                <i>
                    <?php echo $lang['currency']; ?></i><?php echo ncPriceFormat($output['goods']['promotion_price']); ?></strong><em><?php echo $lang['currency'] . ncPriceFormat($output['goods']['goods_price']); ?></em>
            <?php } else {?>
    ...
    foreach ((array) $output['goods']['jjg_info']['levels'] as $levelId => $v) {?>
      ...
      <?php }?>
      <?php if (!$output['goods']['gc_id'] == '1069') {?>
          <dl class="rate">
          <!-- S 描述相符评分 -->
      ...
      <?php }?>
      <div class="ncs-key" style="display: none;">
...
<?php } else {?>
	...
	if (typeof chain_id == 'number') {
		$('#buynow_form').append('<input type="hidden" name="spec_r" value="我终于找到你啦！！"><input type = "hidden" name="begindate" value="开始日期"><input type="hidden" name="enddate" value="结束日期"><input type="hidden" name="ifchain" value="1"><input type="hidden" name="chain_id" value="'+chain_id+'"><input type="hidden" name="area_id" value="'+area_id+'"><input type="hidden" name="area_name" value="'+area_name+'"><input type="hidden" name="area_id_2" value="'+area_id_2+'">');
  }
  <?php if ($output['goods']['gc_id_1'] == 1){ ?>
    <?php $str = "入住日期 ".json_decode($_COOKIE['getHotelPrice'])->start."  退房日期 ".json_decode($_COOKIE['getHotelPrice'])->end ?>
  $('#buynow_form').append('<input type="hidden" id = "spec_r" name="spec_r" value="<?php echo $str ?>"><input type = "hidden" id="begindate" name="begindate" value="lala日期"><input type="hidden" id="enddate" name="enddate" value="结束日期"> ');
  var result = "入住日期 " + $('#date_start').val() + "  退房日期 " + $('#date_end').val();
  var begindate = $('#date_start').val();
  var enddate = $('#date_end').val();
  $('#spec_r').val(result);
  $('#begindate').val(begindate);
  $('#enddate').val(enddate);
  console.log($('#spec_r').val());
  console.log  ($('#begindate').val());
  console.log  ($('#enddate').val());
  <?php }?>
    <?php if($output['goods']['gc_id_1'] == 2){ $str = "预定日期"?>

    $('#buynow_form').append('<input type="hidden" id = "spec_r" name="spec_r" value="<?php echo $str ?>">');
    var result = "会议日期" + $('#meetingdate').val();
    $('#spec_r').val(result);
    <?php }?>
    $("#buynow_form").submit();
<?php }?>
...
function getHotelPrice(){
    var start = $('#date_start').val(),
        ...
        goods_id = <?=$output['goods']['goods_id']?>,
        data = {};

    data.start = $('#date_start').val();
    data.end = $('#date_end').val();
    data.goods_id = <?=$output['goods']['goods_id']?>;
    addCookie("getHotelPrice",JSON.stringify(data));
    var data1 = {};
    data1 =getCookie("getHotelPrice");
    console.log(data1);
    //
    ...
    $.ajax({
        type:'post',
        url:'/shop/index.php?act=test&op=price',
        data:data,
        dataType:'json',
        success:function(result) {
            ...
            if (result.data.price) {
                ...
                $('#price_goods').html(result.data.price)
            }
