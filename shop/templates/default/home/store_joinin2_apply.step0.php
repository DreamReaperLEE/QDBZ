...
<!--        <a id="btn_apply_agreecbc_next" href="javascript:;" class="btn">个人入驻</a>-->
        <a id="btn_apply_agreement_next" href="javascript:;" class="btn">供应商入驻</a>
        <a style=" margin-left:15px;" id="btn_apply_agreement_next2" href="javascript:;" class="btn">保障单位入驻</a>
    </div>
</div>
...
$(document).ready(function(){
    ...
    $('#btn_apply_agreement_next2').on('click', function() {
        if($('#input_apply_agreement').prop('checked')) {
            window.location.href = "index.php?act=store_joinin2&op=step1";
        } else {
            alert('请阅读并同意协议');
        }
    });
