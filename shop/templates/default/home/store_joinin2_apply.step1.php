...
<div id="apply_company_info" class="apply-company-info">
  ...
  <form id="form_company_info" action="index.php?act=store_joinin2&op=step2" method="post">
    <table border="0" cellpadding="0" cellspacing="0" class="all">
      <thead>
        <tr>
          <th colspan="2">单位及联系人信息</th>
        </tr>
      </thead>
      <tbody>
        <tr>
    ...
    <table border="0" cellpadding="0" cellspacing="0" class="all">
      <thead>
        <tr>
           <th colspan="20">附件：</em></th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <th>附件名称：</th>
          <td><input name="enclosure_name" type="text" class="w200"/>
            <span></span></td>
        </tr>
        <tr>
          <th>附件：</th>
          <td><input name="enclosure_electronic" type="file" class="w60"/>
              <input name="enclosure_delete" type="button" value="删除" />
            <span class="block">请确保图片清晰。</span>
            <input name="enclosure_electronic1" type="hidden"/><span></span>
            </td>
        </tr>
      </tbody>
      <tfoot>
...
$(document).ready(function(){
	<?php foreach (array('business_licence_number_elc','enclosure_electronic','general_taxpayer') as $input_id) { ?>
    $('input[name="<?php echo $input_id;?>"]').fileupload({
        dataType: 'json',
        url: '<?php echo urlShop('store_joinin2', 'ajax_upload_image');?>',
        formData: '',
        add: function (e,data) {
            data.submit();
        },
    ...
    $('input[name="enclosure_delete"]').on('click',function(){
        $('input[name="enclosure_name"]').val("");
        if($('input[name="enclosure_electronic"]').next().prop("tagName") == 'IMG')
        {
            $('input[name="enclosure_electronic"]').next().remove();
        }
    });
