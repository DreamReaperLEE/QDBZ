...
>
        ...
        <td class="tl" <?php if ($cart_info['bl_id'] != '0') {?>colspan="2"<?php }?>><dl class="ncc-goods-info">
            ...
            <!-- 修改 -->
            <dd class="goods-spec">
            <?php //print_r($cart_info);?>
            <?php print_r($output['spec_r']) ?></dd>
            <!-- 终止 -->
            <?php } ?>
            <?php if ($cart_info['bl_id'] != '0') {?>
      ...
      <?php } ?>
      <tr>
        <td colspan="20"><div class="ncc-msg">联系方式：
            <input id="mob_phone" name="pay_message[<?php echo $store_id;?>]" class="number" data-msg-required="*手机号不能为空" placeholder="请留下您的联系方式" title="选填：对本次交易的说明（建议填写已经和商家达成一致的说明）"  maxlength="150"></input>
          </div>
...
function submitNext(){
  ...
  SUBMIT_FORM = false;
  $('#order_form').append('<input type="hidden" name="spec_r" value="<?php echo $output['spec_r']; ?>"> ');
 	$('#order_form').submit();
}
