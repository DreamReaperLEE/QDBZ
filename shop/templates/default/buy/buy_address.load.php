...
<ul>
  ...
  <li class="receive_add address_item <?php echo $k == 0 ? 'ncc-selected-item' : null; ?>">
    ...
    <label for="addr_<?php echo $val['address_id']; ?>"><span class="true-name"><?php echo $val['true_name'];?></span><span class="address"><?php echo $val['area_info']; ?>&nbsp;<?php echo $val['address']; ?></span><?php echo $val['mob_phone'] ? $val['mob_phone'] : $val['tel_phone'];?></span></label>
...
<!--    <label for="add_addr">使用新地址</label>-->
    ...
    &nbsp;<label><a class="del" href="<?php echo urlMember('member_address','address');?>" target="_blank">管理项目</a></label>
    <?php } ?>
  </li>
  <?php if ($_GET['ifchain']) { ?>
  <li class="receive_add addr_item">
