...
<script> (function() {window.arr = null;})() </script>
<div class="wrap">
  <div class="tabmenu">
    ...
    <table class="ncm-search-table">
      ...
      <tr>
        <td>&nbsp;</td>
          ...
          <?php /*  $street = array();*/?>
          <?php /*foreach ($output['order_group_list'] as $order_pay_sn1 => $group_info1) { */?>
          <?php /*  $orderlist1 = $group_info1['order_list'];*/?>
          <?php /*  $orderlist2 = $orderlist1[0];*/?>
          <?php /*  $orderlist3 = $orderlist2['reciver_info'];     */?>
          <?php /*  $orderlist4 = $orderlist3['street'];     */?>
          <?php /*  $street[$orderlist4] = $orderlist4;     }*/?>
          <script type="text/javascript">
            var $list = "<?php /*echo $street; */?>" ;
            alert($list);
          </script>-->
          <th><?php echo $lang['member_order_project'];?></th>
          <td class="w100"><select name="project_name_type">
  ...
  <table class="ncm-default-table order">
    ...
    <tbody <?php if (!empty($group_info['pay_amount']) && $p == 0) {?> class="pay" <?php }?>>
      ...
      <?php }?>
      <?php $p++;?>
      <tr  class="<?php echo $order_info['extend_order_common']['reciver_info']['street'] ?>" >
        <th colspan="19"> <span class="ml10"> 
          ...
          <?php echo $lang['member_order_sn'].$lang['nc_colon'];?><?php echo $order_info['order_sn']; ?>
               <!-- fix -->
                ...
                <script> if(arr == null) {
                    ...
                    arr.add(                '<?php print_r($order_info['extend_order_common']['reciver_info']['street']) ?>' )
                </script>
          ...
          <span><a href="<?php echo urlShop('show_store','index',array('store_id'=>$order_info['store_id']), $order_info['extend_store']['store_domain']);?>" title="<?php echo $order_info['store_name'];?>"><?php echo $order_info['store_name']; ?></a></span>
            ...
            <!-- 管理员显示id -->
            <span><?php if ($output['$isadmin1']==1){   echo "买家："; echo $order_info['buyer_name']; }?></span>
...
//"$output['order_group_list']
function getProjectName(arr){
        alert(arr);
        foreach()
...
arr.forEach(function(item) {
    $('[name="project_name_type"]')[0].options.add(new Option(item,item))
})

//新增部分 动态控制显示
if(Boolean("<?php echo $output['project_name_type'] ?>")) {
    var lista = Array.from($("tbody"))
    lista.forEach(function(item) {item.style="display: none"})

    var list = Array.from($(".<?php echo $output['project_name_type'] ?>"))
    lista[0].style="display: "
    list.forEach(function(item) {item.parentNode.style="display: "})
