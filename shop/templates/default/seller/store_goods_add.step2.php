  ...
  <form method="post" enctype="multipart/form-data" id="goods_form" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_goods');} else { echo urlShop('store_goods_add', 'save_goods');}?>">
    ...
    <div class="ncsc-form-goods">
      ...
      <dl>
        ...
        <dd id="gcategory"> <?php echo $output['goods_class']['gc_tag_ name'];?> <a class="ncbtn" href="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_class', array('commonid' => $output['goods']['goods_commonid'], 'ref_url' => getReferer())); } else { echo urlShop('store_goods_add', 'add_step_one'); }?>"><?php echo $lang['nc_edit'];?></a>
      ...
      <dl>
          <p>
          <button type="button">设定价格</button><a href="<?php echo urlShop('hotel', 'edit_hotel', array('commonid' => $val['goods_commonid']));?>" class="btn-bluejeans"><i class="icon-edit"></a>
          </p>
      </dl>
      <dl>
...
function recursionSpec($len,$sign) {
    if($len < $sign){
        ...
        echo "str +='<td><input class=\"text price\" type=\"text\" name=\"spec['+spec_bunch+'][marketprice]\" data_type=\"marketprice\" nc_type=\"'+spec_bunch+'|marketprice\" value=\"\" /><em class=\"add-on\"><i class=\"icon-renminbi\"></i></em></td>' +
                    ...
                    '<td><input class=\"text sku\" type=\"button\" name=\"spec['+spec_bunch+'][barcode]\" nc_type=\"'+spec_bunch+'|barcode\" value=\"aaa\" /></td>' +
                    '</tr>';\n";
        for($i=0; $i< $len; $i++){
            echo "}\n";
        }
